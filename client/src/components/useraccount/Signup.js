...
const Signup = () => {
  ...
  const signupForm = async (formValues) => {
    

    let body = { email: formValues.email };
    let userRegistered = false;

    const salt = await bycrypt.genSalt(12);
    const hashedPswd = await bycrypt.hash(formValues.password, salt);

    axios
      .post(constant.BE_URL + "userdetails/isUserRegistered", body)
      .then((response) => {
        if (response.data.userexist) {
          toast.error("User already registered with the given email address.");
        } else {
          let bodyData = {
            userdata: {
          ...
          axios
            .post(constant.BE_URL + "userdetails/registerUser", bodyData)
            .then((response) => {
