...
const Header = (props) => {
  ...
  const  signout=()=>{
    ...
    axios.post(constant.BE_URL + "userdetails/signOut", bodyData,{ headers: {"Authorization" : `Bearer ${token}`} })
    .then(
      response => {
        
        if(response.data.tokenDeactivated){
    ...
    .catch(error => {
      console.error('There was an error!', error);
      toast.error("Error while logout from account");
    });
